...
io.on("connection", (socket) => {
  ...
  const getSchedule = () => {
    const query =
      "SELECT schedule.*, post.* FROM schedule INNER JOIN post ON schedule.schedID = post.schedID";
    db.query(query, (error, data, field) => {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        const schedDate = convertToDateObject(
          data[i].schedDate,
          data[i].schedTime
        );
        const dueDate = convertToDateObject(data[i].dueDate, data[i].dueTime);
        const closeDate = convertToDateObject(
          data[i].closeDate,
          data[i].closeTime
        );
        ...
        schedule.scheduleJob(schedDate, function () {
          ...
          const query =
            "UPDATE post SET timePosted=?, datePosted=?, schedStatus='no' WHERE schedID=?";
          db.query(
            query,
            [timePosted, datePosted, schedID],
            (error, data, field) => {
              if (error) {
                console.log(error);
              }
        ...
        schedule.scheduleJob(closeDate, function () {
          ...
          //QUERY TO UPDATE POST TABLE
          const query =
            "UPDATE post SET dueStatus='no', closeStatus='yes' WHERE schedID=?";
          db.query(query, [schedID], (error, data, field) => {
            if (error) {
              console.log(error);
            }
